function createProduct(){
    ...
    modalContent.innerHTML = `
        <div class="modal-header">
            <h5 class="modal-title" id="CreateModal">Create Product</h5>
        ...
        <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary" style="background-color: orange; color: white;" onclick="Create()">Create</button>
        </div>
    `;
...
function Create(){
    ...
    fetch(`http://localhost:8000/products`, requestOptions)
        ...
        .then(data => {
            ...
            row.innerHTML = `
                <td>${data.data.id}</td><td>${data.data.attributes.nome}</td><td>${data.data.attributes.marca}</td><td>${data.data.attributes.prezzo}</td>
                <td>
